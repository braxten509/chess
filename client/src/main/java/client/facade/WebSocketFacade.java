...
/**
 * Receives requests from Client to upgrade the connection to a WebSocket connection. Responses are
 * sent to NotificationHandler
 */
public class WebSocketFacade extends Endpoint {
  ...
  /**
   * Upgrades client connection to a Websocket connection. First: formats url to -> ws://{url}/ws
   * (as a URI object). Second: makes a new session identified by the URI. Third: onMessage sends
   * that message to a NotificationHandler when WebSocketFacade receives a message.
   *
      ...
      this.session.addMessageHandler(
          new MessageHandler.Whole<String>() {
            ...
            public void onMessage(String jsonMessage) {
              try {
                printf(
                    "(Client.WebSocketFacade::onMessage) received message",
                    SpacingType.REGULAR,
                    SET_TEXT_COLOR_LIGHT_GREY);
                chessMessageHandler.handleMessage(jsonMessage);
                printf(
                    "(Client.WebSocketFacade::onMessage) executed successfully",
                    SpacingType.UNDER,
                    SET_TEXT_COLOR_LIGHT_GREY);
              } catch (Exception e) {
                printf(
                    "\n(Client.WebSocketFacade::onMessage) got an error: " + e.getMessage(),
  ...
  /**
   * What the WebSocketFacade does after a connection is opened. Necessary for interface, but unused
   * in this code.
   *
