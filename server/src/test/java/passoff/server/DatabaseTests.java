...
public class DatabaseTests {
  ...
  public void persistenceTest() {
    ...
    TestCreateResult createResult = serverFacade.createGame(
      new TestCreateRequest(gameName),
      auth
    );

    //join the game
    serverFacade.joinPlayer(
      new TestJoinRequest(ChessGame.TeamColor.WHITE, createResult.getGameID()),
      auth
    );

    Assertions.assertTrue(
      initialRowCount < getDatabaseRows(),
      "No new data added to database"
    );
    ...
    Assertions.assertEquals(
      ...
      "Missing game(s) in database after restart"
    );
    ...
    Assertions.assertEquals(
      gameName,
      game1.getGameName(),
      "Game name changed after restart"
    );
    Assertions.assertEquals(
      TEST_USER.getUsername(),
      game1.getWhiteUsername(),
  ...
  private void checkTableForPassword(String table, Connection connection)
    throws SQLException {
      ...
      while (rs.next()) {
        for (int i = 1; i <= columns; i++) {
          ...
          Assertions.assertFalse(
            value.contains(TEST_USER.getPassword()),
  ...
  private Class<?> findDatabaseManager() throws ClassNotFoundException {
    ...
    throw new ClassNotFoundException(
      "Unable to load database in order to verify persistence. " +
