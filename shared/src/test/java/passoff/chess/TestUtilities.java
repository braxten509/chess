...
public class TestUtilities {
  ...
  public static void validateMoves(
    ...
    var pieceMoves = new ArrayList<>(
      testPiece.pieceMoves(board, startPosition)
    );
    validateMoves(validMoves, pieceMoves);
  }

  public static void validateMoves(
    List<ChessMove> expected,
    List<ChessMove> actual
  ) {
    Comparator<ChessMove> comparator = Comparator.comparingInt(
      TestUtilities::moveToInt
    );
  ...
  private static final Map<Character, ChessPiece.PieceType> CHAR_TO_TYPE_MAP =
    Map.of(
      'p',
  ...
  public static List<ChessMove> loadMoves(
    ChessPosition startPosition,
    int[][] endPositions
  ) {
    ...
    for (var endPosition : endPositions) {
      validMoves.add(
        new ChessMove(
          startPosition,
          new ChessPosition(endPosition[0], endPosition[1]),
          null
        )
      );
    }
  ...
  private static int moveToInt(ChessMove move) {
    return (
      1000 * positionToInt(move.getStartPosition()) +
      10 * positionToInt(move.getEndPosition()) +
      ((move.getPromotionPiece() != null)
          ? move.getPromotionPiece().ordinal() + 1
          : 0)
    );
  }
}
